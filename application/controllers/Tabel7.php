...
class Tabel7 extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->load->model('Tabel7Model');
        $this->path = 'assets/images/';
    }

	public function index()
	{
        $tabel7s = $this->Tabel7Model->show();
      
        $this->load->view('main.php',[
            "page" => "tabel7",
			"title" => "Tabel7",
			"subtitle" => "PkM DTPS yang Melibatkan Mahasiswa",
            "content" => [],
            "tabel7s" => $tabel7s, 
        ]);
    }
    ...
    public function add()
    {
        ...
        $this->Tabel7Model->insert($data);
        redirect(base_url('Tabel7'));
    ...
    public function edit()
    {
        ...
        $this->Tabel7Model->update($data, $id);
        redirect(base_url('Tabel7'));
    }

    public function delete($id)
    {
        $this->Tabel7Model->delete($id);
        redirect(base_url('Tabel7'));
	}
	...
	public function import() 
	{
		if ($this->input->post('submit')) {
			...
			if(empty($error)){
				...
				$this->Tabel7Model->insert_batch($datains);
        		redirect(base_url('Tabel7'));           
			} catch (Exception $e) {
